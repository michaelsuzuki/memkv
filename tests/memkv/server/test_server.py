...
def assert_correct_response(server: Server, msg: MessageT, response: pb2.Response):
    if isinstance(msg, pb2.GetCommand):
        ...
        for key, actual_value in msg_keys_dict.items():
            ...
            expected_value = (
                server.key_value_store[key] if key in server.key_value_store else None
            )
            assert (
...
async def create_metrics(
    server: Server, loop: AbstractEventLoop
) -> pb2.MetricsResponse:
    server.key_value_store["keyOne"] = b"valueOne"
    server.key_value_store["keyTwo"] = b"valueTwo"
    unique_keys = set()
    ...
    total_size = reduce(
        lambda a, b: a + b, [len(kv.value) for kv in set_cmd.key_values]
    )
...
async def test_handle_metrics_command(metrics_msg, event_loop: AbstractEventLoop):
    ...
    try:
        ...
        assert (
            expected_metrics.keys_updated_count == response.metrics.keys_updated_count
        )
        assert (
            expected_metrics.keys_deleted_count == response.metrics.keys_deleted_count
        )
        assert (
            expected_metrics.total_store_contents_size
            == response.metrics.total_store_contents_size
        )
    except Exception:
        raise
    finally:
